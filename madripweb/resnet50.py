...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, norm_layer=None,
        ...
        if self.liu:
            ...
            self.branch_bam1 = cbam.CBAM(512 * block.expansion)
            self.branch_bam2 = cbam.CBAM(512 * block.expansion)
            self.classifier_dep1 = nn.Linear(512 * block.expansion, 1024)
            self.classifier_dep2 = nn.Linear(512 * block.expansion, 1024)
            self.branch_bam3 = cbam.CBAM(1024, no_spatial=True)
            self.branch_bam4 = cbam.CBAM(1024, no_spatial=True)
            ...
            self.branch_bam3 = cbam.CBAM(2048)
            self.branch_bam4 = cbam.CBAM(2048)
